...
const signup = async (req, res, next) => {
  ...
  await UserModel.create({ login: user.login, email: user.email, password: hashedPassword })
...
const signin = async (req, res, next) => {
  ...
  const accessToken = jwt.sign(
    { id: user.id, login: user.login }, config.env.JWT_SECRET, { expiresIn: config.env.tokenExpiration });

  const refreshToken = jwt.sign(
    { id: user.id, login: user.login }, config.env.JWT_REFRESH_SECRET);
...
const getMe = async (req, res, next) => {
  ...
  return res.status(200).json({ id: user.id, login: user.login, email: user.email });
}
